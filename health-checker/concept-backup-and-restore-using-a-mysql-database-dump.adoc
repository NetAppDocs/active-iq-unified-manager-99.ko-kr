...
[NOTE]
====
Linux 및 Windows 서버에 Unified Manager가 설치된 경우 MySQL 데이터베이스 덤프는 기본 백업 메커니즘입니다. Red Hat Enterprise Linux, CentOS Linux 시스템 또는 Windows의 경우 Active IQ Unified Manager가 많은 수의 클러스터와 노드를 관리하고 있거나 MySQL 백업을 완료하는 데 몇 시간이 걸리는 경우 NetApp Snapshot 백업 방법을 사용할 수 있습니다.
